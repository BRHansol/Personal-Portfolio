...
async def health_check():
    ...
    # Determine overall status (exclude self-referential gateway entry)
...
def _parse_processing_options(processing_options: Optional[str]) -> ProcessingRequest:
    """Parse optional JSON form field used with multipart upload."""
    if not processing_options or not processing_options.strip():
        return ProcessingRequest()
    try:
        return ProcessingRequest.model_validate_json(processing_options)
    except Exception:
        data = json.loads(processing_options)
        return ProcessingRequest.model_validate(data)
...
async def process_file_endpoint(
    ...
    try:
        ...
        if opts.enable_processing:
            ...
            processing_result = await process_file(file_id, opts.processing_operation)
        
        # Step 3: AI analysis (if enabled)
        ai_result = None
        if opts.enable_ai_analysis:
            logger.info(f"Step 3: AI analysis for file {file_id}")
            ai_result = await analyze_file(file_id, opts.ai_analysis_type)
